...
def is_text_component(symbol_crop, min_area=40):
    """
    Detects if a connected component is likely TEXT rather than a symbol.
    Text tends to be: thin/tall, low pixel density, or contains straight horizontal lines.
    """
    ...
    # Text characters are often very thin (I, l, 1) or very wide (-, _)
    if aspect_ratio < 0.3 or aspect_ratio > 3.5:
        return True
        
    # Check pixel density: symbols are usually denser than text
    total_pixels = h * w
    dark_pixels = np.sum(symbol_crop < 128)
    density = dark_pixels / total_pixels
    
    # Text typically has lower density (< 0.15) compared to electrical symbols
    if density < 0.12:
        return True
        
    # Check for long horizontal runs (typical of underscores or hyphens in text)
    row_sums = np.sum(symbol_crop < 128, axis=1)
    max_row_fill = np.max(row_sums) / w if w > 0 else 0
    if max_row_fill > 0.85 and h < 20:  # Very wide relative to height = likely text line
        return True
        
    return False

def extract_symbols_from_legend(legend_img, category_filter="All"):
    """Extracts ONLY graphical symbols, filtering out text/letters."""
    try:
        ...
        for i in range(1, num_labels):
            ...
            # CRITICAL: Filter out text/letters
            if is_text_component(symbol_crop):
        ...
        print(f"Extraction complete: {len(extracted_items)} symbols kept, {text_filtered_count} text items filtered out.")
...
def multi_scale_match(drawing_gray, template, threshold, scales=None):
    ...
    if scales is None:
        # Reduced set of scales for speed
...
if legend_file and preview_btn:
    with st.spinner("Extracting symbols from legend..."):
        ...
        if legend_img:
            ...
            if all_templates:
                st.success(f"✅ Extracted {len(all_templates)} TRUE SYMBOLS (text/letters filtered out).")
        ...
        try:
            ...
            accumulator = {
                item["name"]: {
                    ...
                    "Count": 0,
                    "template": item["template"]  # Store template directly for speed
                }
